...
class GaussianDreamerConfig:
    ...
    # prompt_processor字段（注意：你注释掉了 prompt_processor_type，但保留 processor 本体）
    # prompt_processor: Dict[str, Any] = field(default_factory=dict)
    #
    # # guidance字段（stable diffusion 用）
...
class GaussianDreamer():
    def __init__(self, cfg: Dict[str, Any]):
        ...
        self.cfg = GaussianDreamerConfig(**cfg)
    ...
    def shape(self):
        ...
        model.load_state_dict(
            torch.load('./load/shapE_finetuned_with_330kdata.pth', map_location=device)['model_state_dict'])
    ...
    def add_points(self, coords, rgb):
        ...
        num_points = 1000000
        points = np.random.uniform(low=np.asarray(bbox.min_bound), high=np.asarray(bbox.max_bound),
                                   size=(num_points, 3))
    ...
    def forward(self, batch: Dict[str, Any], renderbackground=None) -> Dict[str, Any]:
        ...
        for id in range(batch['c2w_3dgs'].shape[0]):
            ...
            image, viewspace_point_tensor, _, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg[
    ...
    def on_fit_start(self) -> None:
        ...
        # print("self.cfg.prompt_processor", self.cfg.prompt_processor)
        # self.prompt_processor = StableDiffusionPromptProcessor(**self.cfg.prompt_processor)
        # self.guidance = StableDiffusionGuidance(**self.cfg.guidance)
