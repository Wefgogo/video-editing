...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    graphencoder = GaussianGraphEncoder().to("cuda")
    ...
    for name, module in unet.named_modules():
        if name.endswith(tuple(trainable_modules)):
            # 跳过 down_blocks 的前两层
            # if name.startswith("down_blocks.0") or name.startswith("down_blocks.1"):
            #     continue
    ...
    optimizer = optimizer_cls(
        list(unet.parameters()) + list(graphencoder.parameters()),
        lr=learning_rate,
    ...
    loss_back_list = []
    loss_total_list = []
    writer = SummaryWriter('runs/tune-video')
    # writer = SummaryWriter('runs/video_editing_detach')
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                #
                # latents_bg = (latents * (1 - mask_latent)).detach()  # 不参与梯度
                # latents_fg = latents * mask_latent  # 前景可训练
                # latents = latents_bg + latents_fg  # 合成作为模型输入
                #------------------------------------------------------------------------------------------
                ...
                # 先计算前景 MSE（按像素平均）
                # mask_for_loss = mask_latent  # (b,1,f,64,64)
                # num_chan = model_pred.shape[1]

                # 计算 sum mse 并对前景像素数量做归一化
                # sq = (model_pred - target) ** 2  # (b,C,f,h,w)
                # masked_sq = sq * mask_for_loss  # broadcasting across channels
                # den = (mask_for_loss.sum() * num_chan).clamp_min(1.0)  # 避免 /0
                # loss_fg = masked_sq.sum() / den
                #
                # # 可选：背景一致性 loss（给背景一个较大的权重 lambda_bg）
                # lambda_bg = 0.0  # 先试 0（冻结已足够），必要时设为 0.1~2.0
                # if lambda_bg > 0:
                #     bg_sq = sq * (1 - mask_for_loss)
                #     den_bg = ((1 - mask_for_loss).sum() * num_chan).clamp_min(1.0)
                #     loss_bg = bg_sq.sum() / den_bg
                #     loss = loss_fg + lambda_bg * loss_bg
                # else:
                #     loss = loss_fg
                # --------------------------------------------------------------------------
                # mask_latent = mask.permute(0, 2, 1, 3, 4)  # (N, 1, D, H_orig, W_orig)
                ...
                if global_step % validation_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        for idx, prompt in enumerate(validation_data.prompts):
                            sample = validation_pipeline(prompt, generator=generator, latents=ddim_inv_latent, depth=depth,
    ...
    # plt.plot(loss_front_list, label='Loss Front (Foreground)')
    # plt.plot(loss_back_list, label='Loss Back (Background)')
    ...
    #
    # # 保存到本地
    # plt.savefig('loss_convergence.png', dpi=300)  # 可以修改路径和文件名
    # plt.close()  # 关闭图，避免占用内存
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/hockey.yaml")
