...
from .DepthAdapter import DepthAdapter
from .SpatioTemporalDepth import DepthFeatureExtractor
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # self.depth_adapter = DepthAdapter(
        #     cin=1,  # 深度图通道数
        #     channels=[320, 640, 1280, 1280],  # 与骨架适配器一致
        #     nums_rb=2,
        #     ksize=1,
        #     sk=True,
        #     use_conv=False
    ...
    def forward(
        ...
        #     depth_features = self.depth_adapter(depth)
        # else:
        #     depth_features = None
        depth_feature = depth
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            # if depth is not None:
            #     depth_feature = depth_features[idx]
            # else:
            #     depth_feature = None
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples = downsample_block(
                    ...
                    cond_adapter=depth_feature,
                )
            else:
                sample, res_samples = downsample_block(hidden_states=sample, temb=emb, cond_adapter=depth_feature,)
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None):
        ...
        # 过滤掉 depth_adapter 的权重
        filtered_state_dict = {k: v for k, v in state_dict.items()
                               if not k.startswith("depth_adapter.")}
